		...
		<div id="grade" style="background-color: #555555">
			<table>
				...
				<tr>
					...
					<td><input type="image" src="<%=list.get(i).getPoster()%>"
						onclick="javascript:clickimg(<%=list.get(i).getSeq()%>,'<%=list.get(i).getName()%>',<%=num%>)"><br>
					<b style="text-align: left;"><%=two(list.get(i).getName())%></b><br><%=list.get(i).getStart_time()%>

						<br> <input type="image" class="img"
						style="margin-left: 20px;" src="../image/btn_movie_like_off.png"
						onclick="location.href='../movie/likeclick.jsp?seq=<%=list.get(i).getSeq()%>&WHERE=../grade/grade.jsp?num=<%=num%>'">
						<%=list.get(i).getHeart()%> <input type="image"
						src="../image/btn_movie_tiketing.png"
						onclick="location.href='tiket.jsp'"></td>
...
%>	
	...
	<%-- 	<input type="button" value="평점 작성"
			onclick="location.href='../movie/gradewriteAf.jsp?seq=<%=seq%>&id=id&title=<%=mov_name%>'"> --%>
