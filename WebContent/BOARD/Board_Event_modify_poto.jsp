...
if(isMultipart){
	...
	while(it.hasNext()){
		...
		if(item.isFormField()){									//isFormField가 참일때
			processFromField(item, out);
			if(item.getFieldName().equals("id")){
				id = item.getString("utf-8");					//id값 얻어오기
			}else if(item.getFieldName().equals("title")){
				title = item.getString("utf-8");				//제목 얻어오기
			}else if(item.getFieldName().equals("content")){
				content = item.getString("utf-8");				//내용 얻어오기
			}else if(item.getFieldName().equals("category")){
				category = item.getString("utf-8");				//내용 얻어오기
			}else if(item.getFieldName().equals("seq")){
				sseq = item.getString("utf-8");				//내용 얻어오기
			...
			if(item.getFieldName().equals("fileload")){
				int idx = item.getName().lastIndexOf("\\");		//맨마지막에 글자를 추가
				if(idx== -1){
...
if(!filename.equals("")){ %>
<%-- <%pageContext.forward("Board_Event_modify.jsp?filename="+filename+"&title="+title+"&content="+content+"&id="+id+"&seq="+seq+"&filename_r="+filename_r);%> --%>
<script type="text/javascript">
alert("수정완료")
location.href="Board_Event_modify.jsp?filename=<%=filename%>&title=<%=title%>&content=<%=content%>&id=<%=id%>&seq=<%=seq%>&filename_r=<%=filename_r%>";
</script>
